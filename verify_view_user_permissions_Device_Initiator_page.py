...
class verify_view_user_permissions_Device_Initiator_page(unittest.TestCase):
    def setUp(self):
    ...
    def test_verify_view_user_permissions_Device_Initiator_page(self):
        driver = self.driver
        result = []
        ...
        # Initiator page
        tolog("Initiator page")
        driver.find_element_by_xpath("/html/body/div[1]/div/div[3]/div[1]/div/div[2]/div/div[1]/div/ul/li[6]/a/span/span").click()
        time.sleep(1)
        driver.find_element_by_xpath("/html/body/div[1]/div/div[3]/div[2]/div/div[2]/div/div[2]/table[2]/tbody/tr[2]/td[1]/input").click()
        time.sleep(3)
        ...
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[2]", False,
                                            "Pass: Add Initiator button is disabled",
                                            "Fail: Add Initiator button is enabled"))
        result.append(testLinkAssertEnabled("//tr[2]/td[5]/div/a", False,
                                            "Pass: Delete Icon is disabled",
                                            "Fail: Delete Icon is enabled"))

        if result.count(True) != len(result):
            tolog('\n<font color="red">Fail: View user permissions is failed on Device_Initiator page </font>')
            tolog(Fail)
        else:
