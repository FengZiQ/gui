...
class verify_view_user_permissions_Administration_Event_page(unittest.TestCase):
    def setUp(self):
    ...
    def test_verify_view_user_permissions_Administration_Event_page(self):
        driver = self.driver
        result = []
        ...
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[1]", False,
                                            "Pass: Event-Refresh button is disabled",
                                            "Fail: Event-Refresh button is enabled"))
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[2]", False,
                                            "Pass: Event-Save button is disabled",
                                            "Fail: Event-Save button is enabled"))
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[3]", False,
                                            "Pass: Event-Clear button is disabled",
                                            "Fail: Event-Clear button is enabled"))
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[4]", False,
                                            "Pass: Event-Search button is disabled",
                                            "Fail: Event-Search button is enabled"))

        if result.count(True) != len(result):
            tolog('\n<font color="red">Fail: View user permissions is failed on Administrator_Event page </font>')
            tolog(Fail)
        else:
