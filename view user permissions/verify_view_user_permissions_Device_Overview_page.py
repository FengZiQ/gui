...
class verify_view_user_permissions_Device_Overview_page(unittest.TestCase):
    def setUp(self):
    ...
    def test_verify_view_user_permissions_Device_Overview_page(self):
        driver = self.driver
        result = []
        ...
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[1]", False,
                                            "Pass: Date and Time Setting button is disabled",
                                            "Fail: Date and Time Setting button is enabled"))
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[2]", False,
                                            "Pass: NTP Setting button is disabled",
                                            "Fail: NTP Setting button is enabled"))
        result.append(testLinkAssertEnabled("//pr-button-bar/div/div/div/button[3]", False,
                                            "Pass: Shutdown/Restart Subsystem button is disabled",
                                            "Fail: Shutdown/Restart Subsystem button is enabled"))


        if result.count(True) != len(result):
            tolog('\n<font color="red">Fail: View user permissions is failed on Device_Overview page </font>')
            tolog(Fail)
        else:
